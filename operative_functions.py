...
def save_order(msg):
    ...
    while (i<n):
        ...
        columns = ['Date','Order_code', 'Steel_grade', 'Thickness', 'Width_coils', 'Number_coils','ID_coil','Price_coils', 'Operations', 'coil_status']
...
def contact_list_br_json(rq_contact_list,agent_directory):
    ...
    agents_df = agents_df.loc[agents_df['Name'] == "browser"]
    browser_jid = agents_df['User name'].iloc[-1]
    contact_list_msg = Message(to=browser_jid)
...
def change_warehouse(launcher_df, my_dir, list_id_coil_agents, *args):
    ...
    if args:
        ...
        print(active_coil_str)
        active_coil_split = active_coil_str[1].split("'")
        print(active_coil_split)
        active_coil_df = pd.DataFrame([], columns = ['coil_id', 'coil_agent_name', 'coil_jid' ,'coil_location'])
        active_coil_df = active_coil_df.append(active_coil_split, ignore_index=True)
...
def agents_data():
   ...
   #                    '', '', '', '', '',
    ...
    #                   '', '', '', '', '', '', '', '', '']}
    #
    # agents_data_df = pd.DataFrame(data=d)
