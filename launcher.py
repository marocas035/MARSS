    ...
    class LABehav(OneShotBehaviour):
        async def run(self):
            ...
            if order_code != "No":
                ...
                la_inform_log_json = la_inform_log_json.to_json(orient="records")
                la_order_log = opf.order_to_log(la_inform_log_json, my_dir)
                await self.send(la_order_log)
                """Active coil agents"""
                r = 'Request contact list'
                rq_contact_list = opf.rq_list_br(my_full_name, r)   #función que solicita a browser contact list
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', '--wait_msg_time', type=int, metavar='', required=False, default=800, help='wait_msg_time: time in seconds to wait for a msg')
